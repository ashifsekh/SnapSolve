...
function handleMouseUp(e) {
  ...
  // Deactivate overlay and capture
  deactivateOverlay();
...
function showAnswerCard(rect, config) {
  ...
  document.addEventListener('mousemove', (e) => {
    ...
    const clampedX = Math.max(10, Math.min(window.innerWidth - card.offsetWidth - 10, x));
    const clampedY = Math.max(10, Math.min(window.innerHeight - card.offsetHeight - 10, y));
...
function calculateCardPosition(rect) {
  ...
  if (rect.x > window.innerWidth / 2) {
    ...
    x = Math.min(rect.x + rect.width + margin, window.innerWidth - cardWidth - margin);
  }
...
function processMarkdown(text) {
  ...
  if (listItems) {
    const listHTML = '<ul>' + listItems.map(item => `<li>${item.slice(2)}</li>`).join('') + '</ul>';
    text = text.replace(/^-\s.*(?:\n-\s.*)*$/gm, listHTML);
  }
...
// Capture and solve (placeholder for now)
